# agent-accept-gate Specification

## Purpose

Hold the caller and defer bridging the caller and agent legs until a real human agent signals readiness, rather than bridging when the agent call is merely answered. This is required because the agent endpoint is an ACD/queue (e.g. Webex Contact Center) that auto-answers the outbound agent call into a queue before a human is present. The accept signal is correlated to the correct caller without a phone-number lookup by relying on the in-band DTMF keypress arriving on the agent leg's own WebSocket connection (already linked to its caller at dial time), with an out-of-band HTTP path as a secondary trigger.
